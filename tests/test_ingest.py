...
class TestLMRunStep:
    def test_from_metrics_csv(self, tmp_path: Path) -> None:
        ...
        steps = LMRunStep.from_metrics_csv(
            path, benchmark_name="lm_quality", residual_mode="standard", run_id="r1"
        )
        assert len(steps) == 2
...
class TestCollectFunctions:
    def _make_dps_tree(self, root: Path) -> None:
        ...
        for mode, dri in [("standard", 0.99), ("hyper_connection", 0.95)]:
            run_id = f"{mode}-r1"
            _write_json(root / "depth_preservation" / mode / run_id / "dps.json", {
                "residual_mode": mode,
                "run_id": run_id,
            ...
            _write_json(root / "depth_preservation" / mode / run_id / "status.json", {
    ...
    def test_collect_dps_legacy_layout(self, tmp_path: Path) -> None:
        ...
        _write_json(tmp_path / "results" / "depth_preservation" / "r1" / "standard_dps.json", {
            "residual_mode": "standard",
            "run_id": "r1",
            "dps_scores": [0.99, 0.98],
            "gps_scores": [0.97, 0.95],
    ...
    def test_collect_niah(self, tmp_path: Path) -> None:
        for mode, success in [("standard", True), ("hyper_connection", False)]:
            _write_json(tmp_path / "natural_niah" / mode / "r1" / "niah_result.json", {
                "mode": mode,
                "success": success,
                "generated_text": "abc",
                "input_length": 100,
    ...
    def test_collect_latency(self, tmp_path: Path) -> None:
        _write_json(tmp_path / "inference_memory" / "r1" / "profile_results.json", {
            "standard": [{"layers": 12, "peak_vram_mb": 100.0, "mean_latency_ms": 5.0, "p50_latency_ms": 5.0, "p99_latency_ms": 6.0, "tokens_per_second": 1000.0}],
            "hyper_connection": [{"layers": 12, "peak_vram_mb": 110.0, "mean_latency_ms": 6.0, "p50_latency_ms": 6.0, "p99_latency_ms": 7.0, "tokens_per_second": 950.0}],
        })
    ...
    def test_collect_lm_summaries(self, tmp_path: Path) -> None:
        for mode in ["standard", "hyper_connection"]:
            _write_json(tmp_path / "lm_quality" / mode / "r1" / "metrics" / "summary.json", {
                "benchmark_name": "lm_quality",
                "residual_mode": mode,
                "run_id": "r1",
                "parameter_count": 1000000,
...
class TestCollectCLI:
    def test_end_to_end(self, tmp_path: Path) -> None:
        ...
        # Create dps data
        _write_json(tmp_path / "depth_preservation" / "standard" / "r1" / "dps.json", {
            "residual_mode": "standard", "run_id": "r1",
            "dps_scores": [0.99, 0.98, 0.97], "gps_scores": [0.95, 0.94, 0.93],
            "dri": 0.98, "gpi": 0.94, "n_tokens": 100000,
        })
        _write_json(tmp_path / "depth_preservation" / "standard" / "r1" / "status.json", {
            "benchmark_name": "depth_preservation", "residual_mode": "standard", "run_id": "r1", "status": "completed"
        })
        # Create lm_quality data
        _write_json(tmp_path / "lm_quality" / "standard" / "r1" / "metrics" / "summary.json", {
            "benchmark_name": "lm_quality", "residual_mode": "standard", "run_id": "r1",
            "parameter_count": 1000, "elapsed_seconds": 10.0, "global_step": 100, "peak_cuda_memory_mb": 256.0,
        })

        out_dir = tmp_path / "out"
        result = subprocess.run(
            [sys.executable, "-m", "scripts.ingest.collect",
             "--root", str(tmp_path), "--out", str(out_dir),
             "--benchmarks", "depth_preservation,lm_quality"],
